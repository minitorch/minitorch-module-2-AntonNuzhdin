...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert 1 - sigmoid(a) == pytest.approx(sigmoid(-a), 1e-6)
    assert sigmoid(0) == 0.5
    assert sigmoid(0.5) > sigmoid(0.2)
...
def test_distribute(x: float, y: float, z: float) -> None:
    ...
    """
    assert mul(z, (add(x, y))) == pytest.approx(add(mul(z, x), mul(z, y)), rel=1e-6)
...
def test_other(a, b, c) -> None:
    ...
    assert is_close(add(a, neg(a)), 0.0)
    if b != 0:
        assert is_close(mul(b + 1e-6, inv(b + 1e-6)), 1.0)
    assert is_close(max(a, a), a)
    assert is_close(add(a, b), add(b, a))
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    for x, y in zip(ls1, ls2):
        s1 += x
        s2 += y
    assert is_close(sum(ls1) + sum(ls2), s1 + s2)
